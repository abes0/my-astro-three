...
export default class App extends TemplateArtwork {
  ...
  fillTextures(texturePosition: any, textureVelocity: any) {
    ...
    for (let k = 0, kl = posArray.length; k < kl; k += 4) {
      ...
      posArray[k + 1] = Math.random() * SIZE - SIZE / 2
      posArray[k + 2] = Math.random() * SIZE - SIZE / 2
      posArray[k + 3] = 0

      // -10 ~ 10
      velArray[k + 0] = Math.random() * 20 - 10
      velArray[k + 1] = Math.random() * 20 - 10
      velArray[k + 2] = Math.random() * 20 - 10
      velArray[k + 3] = Math.random() * 20 - 10
    }
