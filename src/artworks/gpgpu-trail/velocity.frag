...
void main() {
    ...
    vel.xyz += spreadRange * vec3(
      snoise( vec4( scatter * pos.xyz, 7.225 + interval * time ) ),
      snoise( vec4( scatter * pos.xyz, 3.553 + interval * time ) ),
      snoise( vec4( scatter * pos.xyz, 1.259 + interval * time ) )
    ) * 0.2;
    vel += -pos * length(pos) * scale;
    vel.xyz *= 0.9 + abs(sin(uv.y * 90.0)) * 0.06;
